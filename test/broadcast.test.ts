...
describe('handleBroadcastRequest', () => {
  it('rejects unauthorized requests', async () => {
    ...
    const res = await handleBroadcastRequest(req, '/broadcast/support/message', io);
  ...
  it('broadcasts support:ticket:updated', async () => {
    ...
    const res = await handleBroadcastRequest(req, '/broadcast/support/ticket-update', io);
    expect(res.status).toBe(200);
  ...
  it('broadcasts support:ticket:assigned', async () => {
    ...
    const res = await handleBroadcastRequest(req, '/broadcast/support/ticket-assigned', io);
    expect(res.status).toBe(200);
  ...
  it('broadcasts support:message:read', async () => {
    ...
    const res = await handleBroadcastRequest(req, '/broadcast/support/message-read', io);
    expect(res.status).toBe(200);
  ...
  it('broadcasts profile:comment:new', async () => {
    ...
    const res = await handleBroadcastRequest(req, '/broadcast/profile/comment', io);
    expect(res.status).toBe(200);
  ...
  it('returns 404 for unknown broadcast route', async () => {
    ...
    const res = await handleBroadcastRequest(req, '/broadcast/unknown', io);
